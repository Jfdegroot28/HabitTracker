...
// All trackable habits for a player = locked core + their custom additions
...
function Login({ auth, roster, onLogin }) {
  ...
  const submit = async () => {
    ...
    if (mode === "coach") {
      ...
      else if (h === p.passHash) onLogin({ role: "player", playerId: p.id, name: p.name });
  ...
  return (
    <div className="pt-gate">
      <Brand tag="Daily Habits & Goals · 2026–2027" />
      <div className="pt-card">
...
function Tracker({ session, roster, saveRoster, auth, changeCoachPw, onLogout, refreshRoster }) {
  ...
  const [selId, setSelId] = useState(isCoach ? roster[0]?.id || null : session.playerId);
  ...
  useEffect(() => { if (selId) loadPlayer(selId); /* eslint-disable-next-line */ }, [selId]);
  ...
  useEffect(() => {
    const t = setInterval(async () => {
      ...
      if (selId) { const p = await sGet("player:" + selId); if (p) setData(p); }
    }, 30000);
  ...
  async function loadPlayer(id) {
    ...
    const stored = await sGet("player:" + id);
    const p = stored || newPlayer(id, entry ? entry.name : "Player");
  ...
  function persist(next) {
    ...
    saveTimer.current = setTimeout(async () => {
      await sSet("player:" + next.id, next); setSaving(false); setSavedAt(Date.now());
    }, 550);
  }
  function update(mut) {
    touch();
    ...
    <>
      ...
      {mainTab === "leaderboard" ? (
        ...
        <div className="pt-empty">
          {isCoach ? <><h2>Add your team</h2><p>Open “Manage” to add players and set their passwords.</p></>
...
function ManageModal({ roster, saveRoster, changeCoachPw, onClose, onPicked }) {
  ...
  const add = async () => {
    ...
    await sSet("player:" + id, newPlayer(id, name.trim()));
  ...
  const remove = async (id) => {
    if (!window.confirm("Remove this player and all their data? Can’t be undone.")) return;
    await saveRoster(roster.filter((x) => x.id !== id));
    await sDel("player:" + id);
  };
...
function Leaderboard({ roster, now }) {
  ...
  useEffect(() => {
    (async () => {
      const entries = await Promise.all(
        roster.map(async (p) => {
          const d = await sGet("player:" + p.id);
          return d ? { ...d, name: p.name } : newPlayer(p.id, p.name);
        })
      );
      setAllData(entries);
    })();
  ...
  const rankings = useMemo(() => {
    ...
    // Collect all habit labels across all players (use first player's habits as reference)
    ...
    // Per-habit breakdown — match habits by label across players
    const allLabels = [...new Set(allData.flatMap((p) => allHabits(p).map((h) => h.label)))];
    const habits = allLabels.map((label) => {
      // find the habit type from any player that has it
  ...
  if (roster.length === 0) {
    return <div className="pt-empty"><h2>No players yet</h2><p>Add players via Manage to see the leaderboard.</p></div>;
  }
