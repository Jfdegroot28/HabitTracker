...
/* storage (shared across everyone who opens the link)
   Permission is requested ONCE at boot via primeStorage().
   After that, shared:true calls proceed silently. */
let _storageReady = false;
async function primeStorage() {
  if (_storageReady || !window.storage) return;
  try {
    // One silent read primes shared-data consent for the whole session
    await window.storage.get("__init__", true);
    _storageReady = true;
  } catch { /* user denied — app falls back gracefully */ }
}
...
export default function App() {
  ...
  useEffect(() => { (async () => {
    if (!window.storage) { setStorageOk(false); setBooted(true); return; }
    await primeStorage();          // request shared-data permission exactly once
    setAuth(await sGet("auth"));
    setRoster((await sGet("roster")) || []);
    setBooted(true);
  })(); }, []);
  ...
  return (
    <div className="pt-root">
      <Style />
      {!storageOk && (
        <div className="pt-warn">Live shared saving is unavailable here — open the published app link so the whole staff shares one tracker and logins persist.</div>
      )}
              ...
              <ul className="pt-hablist">
                {CORE_HABITS.map((h) => (
                  <li key={h.id} className="core">
                    ...
                    {h.label}
                    {h.type === "nutrition" && <span className="pt-hab-badge">1–5 rating</span>}
...
function DayModal({ dateKey, data, now, isCoach, onClose, update, onTouch }) {
  ...
  const status = dayStatus(date, now);
  const entry = data.days[dateKey] || { checks: {}, log: "", nutrition: 0, game: false, gameNotes: "" };
  ...
  const ensureEntry = (p) => { p.days[dateKey] ||= { checks: {}, log: "", nutrition: 0, game: false, gameNotes: "" }; };
